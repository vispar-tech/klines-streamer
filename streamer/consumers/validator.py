"""Validator consumer for comparing local klines to exchange APIs in real-time."""
...
class BaseValidator(BaseConsumer, ABC):
    """Base validator for comparing local klines to exchange APIs."""
    ...
    async def consume(
        ...
        """
        For every local kline, compare basic fields to exchange API's result.
        ...
        if session is None:
            self.logger.warning(f"{self.__class__.__name__}: No HTTP session available")
            return
    ...
    async def _validate_single_candle(
        ...
        """Fetch exchange's authoritative kline for this candle; compare main fields."""
        async with sem:
            ...
            remote = await self._fetch_kline(
                channel, symbol, interval_ms, timestamp, session
            )
            if remote is None:
                return None

            exchange_row = self._map_kline(remote)

            diffs = self._compare_kline_fields(local, exchange_row)
            if diffs:
                return {
                    "symbol": symbol,
    ...
    def _map_kline(self, remote: list[int | float]) -> Dict[str, Optional[Any]]:
        """Map exchange response to standardized kline format."""
        fields: List[str] = [
            "start",
            "open",
            "high",
    ...
    def _compare_kline_fields(
        self, local: Dict[str, Any], exchange_row: Dict[str, Optional[Any]]
    ) -> List[Dict[str, Any]]:
        compare_keys: Sequence[str] = ("open", "high", "low", "close", "volume")
        diffs: List[Dict[str, Any]] = []
        for key in compare_keys:
            lv = local.get(key)
            bv = exchange_row.get(key)
            try:
...
class BybitValidator(BaseValidator):
    """Validator for Bybit exchange API."""
    ...
    async def _fetch_kline(
        ...
        try:
            ...
            async with session.get(self.BYBIT_KLINE_ENDPOINT, params=params) as resp:
                if resp.status != 200:
                    return None
...
class BingxValidator(BaseValidator):
    """Validator for BingX exchange API."""

    BINGX_KLINE_ENDPOINT = "https://open-api.bingx.com/openApi/swap/v2/quote/klines"
    ...
    async def _fetch_kline(
        ...
        try:
            interval = Interval(interval_ms)
            interval_str = interval.to_bingx()
            params = {
                "symbol": str(symbol),
                "interval": interval_str,
            ...
            async with session.get(self.BINGX_KLINE_ENDPOINT, params=params) as resp:
                if resp.status != 200:
                    return None
                data = await resp.json()
                if data.get("code") != 0:
                ...
                if not klines:
                    return None
                return cast(list[int | float], klines[0])
        except Exception:
            self.logger.debug("Kline fetch error")
            return None
...
ValidatorConsumer = _get_validator()
